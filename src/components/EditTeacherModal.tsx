...
import { useTeacherMutations, useTeacherAssignments, useTeacherHasPayments } from '../lib/hooks'
...
export function EditTeacherModal({
  ...
  const { data: hasPayments } = useTeacherHasPayments(teacher?.id || '', { enabled: !!teacher })

  // Calculate counts for validation messages
  const validationCounts = useMemo(() => {
    if (!assignments) return { activeEnrollment: 0, activeService: 0, total: 0 }
  ...
  // Can delete only if no active assignments and no payroll history (either system)
  const canDelete = validationCounts.total === 0 && !hasPayments
  ...
  if (hasPayments) {
    deleteDescriptionParts.push('Cannot delete — has payroll history. Deactivate this teacher instead.')
  }
